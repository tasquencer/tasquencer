/**
 * Check Confirmation Needed Task Route - Domain-First Routing
 *
...
function CheckConfirmationNeededTaskComponentFactory(
  bookings: BookingSummary[],
  onRedirectStart?: () => void
) {
  return createPsaTaskComponent({
    workflowTaskName: "checkConfirmationNeeded",
        ...
        <>
          <div className="rounded-lg border bg-muted/50 p-4">
            <p className="text-sm text-muted-foreground">
              This step checks whether any bookings still need confirmation.
            </p>
          </div>

          {bookings.length === 0 ? (
            <p className="text-sm text-muted-foreground">
              No project bookings were found to evaluate.
            </p>
          ) : (
            <>
    ...
    title: "Check Confirmation Needed",
    description: "Verify whether bookings require confirmation",
    formTitle: "Confirmation Check",
    formDescription:
      "Select the bookings to evaluate for confirmation needs.",
    submitButtonText: "Run Confirmation Check",
    onSuccess: ({ navigate }) => {
      onRedirectStart?.();
...
export const Route = createFileRoute(
  ...
  component: CheckConfirmationNeededTask,
});
 ...
 */
function CheckConfirmationNeededTask() {
  const { projectId } = Route.useParams() as { projectId: Id<"projects"> };
  const [isRedirecting, setIsRedirecting] = useState(false);

  const workItem = useQuery(
    api.workflows.dealToDelivery.api.workItems.getWorkItemByProjectAndType,
    { projectId, taskType: "checkConfirmationNeeded" }
  );
    ...
    return (
      <Card>
        <CardContent className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <h3 className="text-lg font-medium">Check completed</h3>
  ...
  const Component = CheckConfirmationNeededTaskComponentFactory(
    bookingSummaries,
